...
LIVE_SLUGS_CACHE: tuple[float, set[str]] = (0.0, {"auto"})
...
def auth_ok(request: Request) -> bool:
    ...
    return auth == f"Bearer {config.API_KEY}" or auth.split()[-1:] == [config.API_KEY]
...
async def responses_stream(parsed, prev, preferred: str | None = None):
    seq = 0
    rid = "resp_" + __import__("uuid").uuid4().hex
    msg_id = "msg_" + __import__("uuid").uuid4().hex
    ...
    in_prog = _response_resource(rid, parsed.model_requested or "auto", int(__import__("time").time()),
